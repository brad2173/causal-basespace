...
@REM REM === COLLECT FOLDER SIZE BEFORE EXPORT (after X: is mapped, before bs.exe is run) ===
@REM setlocal enabledelayedexpansion
@REM set "TMP_SIZE_BEFORE_GB="
@REM if exist "X:\" (
@REM     for /f %%S in ('powershell -Command "$size = (Get-ChildItem -Path '\\i110filesmb.hs.it.vumc.io\CAUSAL-DataExport' -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum; if ($null -eq $size) { $size = 0 }; $sizeGB = [math]::Round($size / 1GB, 2); Write-Output $sizeGB"') do set "TMP_SIZE_BEFORE_GB=%%S"
@REM     if "!TMP_SIZE_BEFORE_GB!"=="" set "TMP_SIZE_BEFORE_GB=0"
@REM ) else (
@REM     echo [ERROR] X: drive is not mapped. Cannot determine folder size before export. 1>&2
@REM     set "TMP_SIZE_BEFORE_GB=0"
@REM )
@REM endlocal & set "SIZE_BEFORE_GB=!TMP_SIZE_BEFORE_GB!"
...
@REM if "%TOTAL_DIFF%" NEQ "0" (
@REM     setlocal enabledelayedexpansion
@REM     set "TMP_SIZE_AFTER_GB="
@REM     for /f %%S in ('powershell -Command "$size = (Get-ChildItem -Path '\\i110filesmb.hs.it.vumc.io\CAUSAL-DataExport' -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum; if ($null -eq $size) { $size = 0 }; $sizeGB = [math]::Round($size / 1GB, 2); Write-Output $sizeGB"') do set "TMP_SIZE_AFTER_GB=%%S"
@REM     if "!TMP_SIZE_AFTER_GB!"=="" set "TMP_SIZE_AFTER_GB=0"
@REM     endlocal & set "SIZE_AFTER_GB=!TMP_SIZE_AFTER_GB!"
@REM ) else (
@REM     set "SIZE_AFTER_GB=%SIZE_BEFORE_GB%"
@REM )
...
REM === VALIDATE SIZE VALUES ===
set "SIZE_DIFF_GB=unknown"
set "TRANSFER_RATE_GBPH=unknown"
set "VALID_SIZE_BEFORE=1"
set "VALID_SIZE_AFTER=1"

REM Check if SIZE_BEFORE_GB is a valid number
echo [DEBUG] SIZE_BEFORE_GB before validation: %SIZE_BEFORE_GB% 1>&2
echo [DEBUG] SIZE_AFTER_GB before validation: %SIZE_AFTER_GB% 1>&2
echo [DEBUG] Checking if SIZE_BEFORE_GB is a valid number... 1>&2
echo %SIZE_BEFORE_GB% | findstr /R "^[0-9]*\.?[0-9]*$" >nul || set "VALID_SIZE_BEFORE=0"
echo [DEBUG] Checking if SIZE_AFTER_GB is a valid number... 1>&2
echo %SIZE_AFTER_GB% | findstr /R "^[0-9]*\.?[0-9]*$" >nul || set "VALID_SIZE_AFTER=0"

if "%VALID_SIZE_BEFORE%"=="1" if "%VALID_SIZE_AFTER%"=="1" (
    REM Both are valid, calculate diff
    for /f %%D in ('powershell -Command "[math]::Round([decimal]'%SIZE_AFTER_GB%' - [decimal]'%SIZE_BEFORE_GB%',2)"') do set "SIZE_DIFF_GB=%%D"
    REM Only calculate transfer rate if SIZE_DIFF_GB is a valid number
    echo %SIZE_DIFF_GB% | findstr /R "^[0-9]*\.?[0-9]*$" >nul && (
    ...
    if "%VALID_SIZE_BEFORE%"=="0" set "SIZE_BEFORE_GB=unknown"
    if "%VALID_SIZE_AFTER%"=="0" set "SIZE_AFTER_GB=unknown"
    set "SIZE_DIFF_GB=unknown"
    set "TRANSFER_RATE_GBPH=unknown"
    echo [ERROR] One or both size values are invalid. Skipping diff and rate calculation. 1>&2
)
...
if "%SIZE_DIFF_GB%"=="unknown" goto :PRINT_UNKNOWN_SUMMARY
echo     GB Diff:    %SIZE_DIFF_GB%
echo     Rate:       %TRANSFER_RATE_GBPH% GB/hour
goto :PRINT_SUMMARY_END
:PRINT_UNKNOWN_SUMMARY
echo     GB Diff:    unknown
echo     Rate:       unknown
:PRINT_SUMMARY_END
echo     Duration:   %JOB_DURATION% (min, sec) [%JOB_DURATION_HOURS% hrs]
echo -------------------------------------------------------
